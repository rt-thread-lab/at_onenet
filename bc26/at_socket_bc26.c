...
static void at_tcp_ip_errcode_parse(int result) //TCP/IP_QIGETERROR
{
    ...
    case 550:
        ...
        break;
    case 551:
        LOG_E("%d : Operation blocked", result);
        break;
    case 552:
        LOG_E("%d : Invalid parameters", result);
        break;
    case 553:
        LOG_E("%d : Memory not enough", result);
        break;
    case 554:
        LOG_E("%d : Create socket failed", result);
        break;
    case 555:
        LOG_E("%d : Operation not supported", result);
        break;
    case 556:
        LOG_E("%d : Socket bind failed", result);
        break;
    case 557:
        LOG_E("%d : Socket listen failed", result);
        break;
    case 558:
        LOG_E("%d : Socket write failed", result);
        break;
    case 559:
        LOG_E("%d : Socket read failed", result);
        break;
    case 560:
        LOG_E("%d : Socket accept failed", result);
        break;
    case 561:
        LOG_E("%d : Open PDP context failed", result);
        break;
    case 562:
        LOG_E("%d : Close PDP context failed", result);
        break;
    case 563:
        LOG_W("%d : Socket identity has been used", result);
        break;
    case 564:
        LOG_E("%d : DNS busy", result);
        break;
    case 565:
        LOG_E("%d : DNS parse failed", result);
        break;
    case 566:
        LOG_E("%d : Socket connect failed", result);
        break;
    // case 567 : LOG_W("%d : Socket has been closed",       result); break;
    case 567:
        break;
    case 568:
        LOG_E("%d : Operation busy", result);
        break;
    case 569:
        LOG_E("%d : Operation timeout", result);
        break;
    case 570:
        LOG_E("%d : PDP context broken down", result);
        break;
    case 571:
        LOG_E("%d : Cancel send", result);
        break;
    case 572:
        LOG_E("%d : Operation not allowed", result);
        break;
    case 573:
        LOG_E("%d : APN not configured", result);
        break;
    case 574:
        LOG_E("%d : Port busy", result);
        break;
    default:
...
static void urc_qiurc_func(struct at_client *client, const char *data, rt_size_t size)
{
    ...
    {
    case 'c':
        urc_close_func(client, data, size);
        break; //+QIURC: "closed"
    case 'r':
...
extern rt_err_t onenet_event_put(const onenet_event_msg_t *msg);
static void urc_onenet_observe_func(struct at_client *client, const char *data, rt_size_t size)
{
    RT_ASSERT(data && size);
    ...
    sscanf(data, "+MIPLOBSERVE: %*d,%d,%d,%d,%d,%d", &msgId, &flag, &objId, &insId, &resId);

    onenet_event_msg_t msg;
    msg.event.event_src = ONENET_EVENT_SRC_OBSERVE;

    msg.event.type.msgId = msgId;
    msg.event.type.objId = objId;
    msg.event.type.insId = insId;
    msg.event.type.resId = resId;
		msg.event.type.flag = flag;
		
    onenet_event_put(&msg);
}

extern void onenet_event_callback(rt_int32_t id);


static void urc_onenet_event_func(struct at_client *client, const char *data, rt_size_t size)
{
    RT_ASSERT(data && size);
    int ref;
    int eventId;
    sscanf(data, "+MIPLEVENT: %d,%d", &ref, &eventId);
    onenet_event_callback(eventId);
}

static void urc_onenet_read_func(struct at_client *client, const char *data, rt_size_t size)
{
    RT_ASSERT(data && size);
    ...
    sscanf(data, "+MIPLREAD: %d,%d,%d,%d,%d", &ref, &msgId, &objId, &insId, &resId);

    onenet_event_msg_t msg;

    msg.event.event_src = ONENET_EVENT_SRC_READ;
...
static void urc_onenet_write_func(struct at_client *client, const char *data, rt_size_t size)
{
    RT_ASSERT(data && size);
    int ref;
    int msgId;
    ...
    int valueType;
    int len;
    char requestdata[256] = {0};
    int flag;
    int index;

    sscanf(data, "+MIPLWRITE: %d,%d,%d,%d,%d,%d,%d,%[^,],%d,%d", &ref, &msgId, &objId, &insId, &resId, &valueType, &len, requestdata, &flag, &index);

    LOG_I("get wirte request the data is %s\n", requestdata);

    onenet_event_msg_t msg;

    msg.event.event_src = ONENET_EVENT_SRC_WRITE;

    msg.event.type.msgId = msgId;
    msg.event.type.objId = objId;
    msg.event.type.insId = insId;
    msg.event.type.resId = resId;
    msg.event.type.len = len;
    msg.event.type.data = requestdata;

    onenet_event_put(&msg);
}

static void urc_onenet_execute_func(struct at_client *client, const char *data, rt_size_t size)
{
    RT_ASSERT(data && size);
    ...
    sscanf(data, "+MIPLEXECUTE: %d,%d,%d,%d,%d,%d,%s", &ref, &msgId, &objId, &insId, &resId, &len, requestdata);
    onenet_event_msg_t msg;
    msg.event.event_src = ONENET_EVENT_SRC_EXECUTE;
...
static void urc_onenet_parameter_func(struct at_client *client, const char *data, rt_size_t size)
{
    RT_ASSERT(data && size);
}

static void urc_onenet_discover_func(struct at_client *client, const char *data, rt_size_t size)
{
    RT_ASSERT(data && size);
    ...
    sscanf(data, "+MIPLDISCOVER: %*d,%d,%d", &msgId, &objId);

    onenet_event_msg_t msg;

    msg.event.event_src = ONENET_EVENT_SRC_DISCOVER;
...
static const struct at_urc urc_table[] =
    {
        ...
        {"+QIURC:", "\r\n", urc_qiurc_func},
#ifdef BC26_USE_ONENET_PROTOCOL
        {"+MIPLEVENT:", "\r\n", urc_onenet_event_func},
        {"+MIPLOBSERVE:", "\r\n", urc_onenet_observe_func},
        {"+MIPLDISCOVER:", "\r\n", urc_onenet_discover_func},
        {"+MIPLPARAMETER:", "\r\n", urc_onenet_parameter_func},
        {"+MIPLWRITE:", "\r\n", urc_onenet_write_func},
        {"+MIPLREAD:", "\r\n", urc_onenet_read_func},
        {"+MIPLEXECUTE:", "\r\n", urc_onenet_execute_func},
#endif
};
